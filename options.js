...
document.getElementById("exportStudyBtn").addEventListener("click",async()=>{try{const r=await studyData("exportStudy"),url=URL.createObjectURL(YTD_EXCEL.toBlob(r.data));const a=document.createElement("a");a.href=url;a.download="句得-学习记录.xlsx";a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);document.getElementById("studyDataStatus").textContent="学习记录已导出。";}catch(e){document.getElementById("studyDataStatus").textContent=e.message;}});
document.getElementById("clearStudyBtn").addEventListener("click",async()=>{if(!window.confirm("确认清空全部自由学习、闯关试卷、作答成绩、时长与练习进度？收藏的词句、笔记和收藏库自评会保留。"))return;try{await studyData("clearStudy");document.getElementById("studyDataStatus").textContent="学习记录已清空，收藏与笔记已保留。";}catch(e){document.getElementById("studyDataStatus").textContent=e.message;}});
