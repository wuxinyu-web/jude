...
var YTD_LAYOUT_TOOLBAR = {
  css: `
        ...
        #resize{position:absolute;left:50%;transform:translateX(-50%);width:clamp(64px,20vw,240px);flex:none;font-size:0;border-radius:4px}
        #resize:after{content:"━━";font-size:13px}#resize:hover{background:#252c24}
        #resize:hover:after{content:"↕ 拖动";font-size:11px}
        #resizeEdge{position:absolute;top:-4px;left:0;width:100%;height:10px;z-index:3;cursor:ns-resize;touch-action:none}
        :host([data-resizing]) iframe{pointer-events:none}
        :host([data-resizing]){user-select:none}
        :host([data-resizing]) #resize{background:#252c24}
