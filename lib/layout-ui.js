...
(() => {
  ...
  if(chrome.runtime.getURL('').startsWith('safari-web-extension:')&&select){select.querySelector('[value=horizontal]').textContent='视频与学习';const help=select.closest('section')?.querySelector('.help');if(help)help.textContent='视频在上，学习区在下。沉浸模式只显示字幕，拖动分隔线可调整比例。';}
