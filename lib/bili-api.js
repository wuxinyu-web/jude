...
var BILI_API = (() => {
  ...
  async function fetchEpisodeInfo(episodeId, { fetchImpl = fetch } = {}) {
    if (!Number.isSafeInteger(episodeId) || episodeId < 1) throw new Error("无效的剧集编号");
    const response = await fetchImpl(`https://api.bilibili.com/pgc/view/web/season?ep_id=${episodeId}`, {credentials:"include"});
    if (!response.ok) throw new Error(`剧集信息请求失败：HTTP ${response.status}`);
    const payload = await response.json();
    if (payload.code !== 0) throw new Error("无法读取剧集信息，请确认已登录且当前账号可播放该集。");
    const data=payload.result;
    const episodes=[...(data?.episodes||[]),...(data?.section||[]).flatMap(s=>s.episodes||[])];
    const episode=episodes.find(e=>Number(e.id)===episodeId);
    if (!episode?.aid || !episode?.cid) throw new Error("未找到当前剧集，可能受账号或地区访问限制。");
    return {episodeId,seasonId:Number(data.season_id),aid:Number(episode.aid),cid:Number(episode.cid),bvid:episode.bvid||"",
      title:[data.title,`第${episode.title}集`,episode.long_title].filter(Boolean).join(" · "),owner:"哔哩哔哩",duration:Number(episode.duration)/1000||0,page:1,pageCount:1};
  }
  ...
  async function fetchSubtitleTracks(
    { aid, cid, bvid, episodeId, seasonId },
    { fetchImpl = fetch, wbi = wbiModule } = {},
  ) {
    const keys = await wbi.fetchWbiKeys({ fetchImpl });
    const params = { aid, cid, bvid };
    if (episodeId) { params.ep_id=episodeId; if(seasonId)params.season_id=seasonId; }
    const url = wbi.signedUrl(PLAYER_URL, params, keys);
