...
if (typeof document !== 'undefined' && globalThis.YTD_PANEL) (() => {
  ...
  if (!root) return;
  let videoId = null, job = null, busy = false, generation = 0, autoApply = null, restoration=Promise.resolve(), automaticBusy=false;
  ...
  async function ensureAutomatic(){
    if(automaticBusy)return;automaticBusy=true;
    try{
      await videoChanged();const c=P.context(),id=videoId,snapshot=generation;
      if(!YTD_PLATFORM.biliParts(id)||!/^(ai-)?zh(?:-|$)/i.test(c.language||'')||busy)return;
      if(job?.status==='completed'){autoApply=null;await P.applyASR(YTD_ASR_CORE.validateResult(job.result,id));return;}
      if(job&&!['failed','cancelled'].includes(job.status)){autoApply=c.generation;return;}
      const saved=await chrome.storage.session.get('local_asr_auto_'+id);if(snapshot!==generation)return;
      if(saved['local_asr_auto_'+id]){if(!job)notice('自动转写尚未完成。请确认本地服务已启动，然后点「重试转写」。');return;}
      await start();
