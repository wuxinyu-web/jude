...
var YTD_ASR_CORE = (() => {
  function validateResult(result, videoId) {
    ...
    const transcript = result.transcript.map(item => {
      if (typeof item.text !== 'string' || !item.text.trim() || item.text.length > 4000 || !Number.isFinite(item.start) || !Number.isFinite(item.duration) || item.start < 0 || item.duration <= 0 || item.start < previous || item.start + item.duration > 10805) throw new Error('转写文本或时间戳无效。');
    ...
    return {transcript,language:'en',source:'local-asr',videoId,...(result.partial===true?{partial:true}:{} )};
  }
  ...
  return {validateResult,alignChinese,alignChineseSentence,readyToApply};
})();
...
if (typeof document !== 'undefined' && globalThis.YTD_PANEL) (() => {
  ...
  async function loadVideo() {
    ...
    videoId=id;job=null;autoApply=null;lastApplied=null;busy=false;const snapshot=++generation;
