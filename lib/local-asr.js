...
if (typeof document !== 'undefined' && globalThis.YTD_PANEL) (() => {
  ...
  function render() {
    const context = P.context(); root.hidden=!YTD_PLATFORM.biliParts(context.videoId)?.bvid;
  ...
  async function loadVideo() {
    ...
    if(!YTD_PLATFORM.biliParts(id)?.bvid)return;
  ...
  async function start(){
    ...
    const id=videoId,snapshot=generation;if(!YTD_PLATFORM.biliParts(id)?.bvid)return;
  ...
  async function ensureAutomatic({retry=false}={}){
    ...
    try{
      ...
      if(!YTD_PLATFORM.biliParts(id)?.bvid||(!/^(ai-)?zh(?:-|$)/i.test(c.language||'')&&!c.partial)||busy)return;
