...
var YTD_ASR_CORE = (() => {
  function validateResult(result, videoId) {
    ...
    const transcript = result.transcript.map(item => {
      if (typeof item.text !== 'string' || !item.text.trim() || item.text.length > 4000 || !Number.isFinite(item.start) || !Number.isFinite(item.duration) || item.start < 0 || item.duration <= 0 || item.start < previous || item.start + item.duration > 5405) throw new Error('转写文本或时间戳无效。');
    ...
    return {transcript,language:'en',source:'local-asr',videoId};
  }
  ...
  return {validateResult,alignChinese};
})();
...
if (typeof document !== 'undefined' && globalThis.YTD_PANEL) (() => {
  ...
  if (!root) return;
  let videoId = null, job = null, busy = false, generation = 0, autoApply = null;
  ...
  function notice(text) { $('localAsrStatus').textContent=text; }
  function render() {
    ...
    $('localAsrStart').textContent=context.source==='local-asr'?'重新转写原声':'转写英文原声';
    ...
    if(job)notice(`${job.message}${running?` · ${Math.floor(Number(job.progress)||0)}%`:''}`);
  }
  async function poll() {
    if(!job||['completed','cancelled','failed'].includes(job.status)||busy)return;
    const id=videoId, snapshot=generation, jobId=job.id;
    try {
      const result=await request('/jobs/'+jobId);
      if(id!==videoId||snapshot!==generation)return;
      job=result;render();
      if(job.status==='completed' && autoApply===P.context().generation){autoApply=null;await P.applyASR(YTD_ASR_CORE.validateResult(job.result,id));render();}
    }catch(error){if(id===videoId&&snapshot===generation)notice(error.message);}
  }
  async function videoChanged() {
    const id=P.context().videoId;
    if(id===videoId){render();return;}
    videoId=id;job=null;autoApply=null;busy=false;const snapshot=++generation;
    ...
    try{const result=await request('/jobs/'+jobId);if(snapshot===generation){job=result;render();}}catch(error){if(snapshot===generation)notice(error.message);}
  }
  $('localAsrStart').addEventListener('click',async()=>{
    if(busy)return;
    const id=videoId,snapshot=generation;busy=true;render();notice('正在连接本地转写服务…');
    try{
      const result=await request('/jobs','POST',{videoId:id});
      await chrome.storage.local.set({['local_asr_job_'+id]:result.id});
      if(snapshot!==generation)return;
      job=result;autoApply=P.context().generation;render();
      if(job.status==='completed'){autoApply=null;await P.applyASR(YTD_ASR_CORE.validateResult(job.result,id));}
    }catch(error){if(snapshot===generation)notice(error.message);}finally{if(snapshot===generation){busy=false;$('localAsrStart').disabled=false;render();}}
  });
  $('localAsrCancel').addEventListener('click',async()=>{
  ...
  globalThis.YTD_ASR_UI={videoChanged,refresh:render,notice};
