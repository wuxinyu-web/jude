...
(()=>{
  ...
  chrome.runtime.onMessage.addListener((m,_sender,reply)=>{
    ...
    if(m.action==='getCurrentTime'){reply({hasVideo:Boolean(v),currentTime:Number(v?.currentTime)||0,paused:v?.paused??true,videoId:YTD_PLATFORM.videoIdFromUrl(location.href)});return false;}
    if(m.action==='seekTo'){const seconds=Number(m.seconds);if(v&&Number.isFinite(seconds)&&seconds>=0){v.currentTime=seconds;reply({success:true});}else reply({success:false});return false;}
  ...
  function mount(){
    ...
    const parent=document.querySelector('.video-toolbar-left')||document.querySelector('.video-toolbar-container')||document.querySelector('.video-info-container');
    if(!parent)return;
