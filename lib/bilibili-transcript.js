...
var YTD_BILIBILI = (()=>{
  ...
  async function fetchTranscript(id,{fetchImpl=fetch,mode='native'}={}){
    try{
      ...
      const info=parts.episodeId?await A.fetchEpisodeInfo(parts.episodeId,{fetchImpl:bounded}):await A.fetchVideoInfo(parts.bvid,{page:parts.page,fetchImpl:bounded});
      ...
      if(!tracks.length)return {success:false,error:needLogin?'BILI_LOGIN_REQUIRED':'BILI_NO_SUBTITLE',message:needLogin?'请先在当前 Chrome 用户配置登录哔哩哔哩，再重新获取字幕。':'这个视频没有可读取的字幕轨，可通过本地英文原声转写生成字幕。'};
