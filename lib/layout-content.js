...
(() => {
  ...
  let host, sheet, player, timer, frameObserver, ancestors = [], height = 45, mode="vertical", stopDrag=()=>{};
  const playerNode = () => document.querySelector('#bilibili-player') || document.querySelector('#bilibiliPlayer') || document.querySelector('.bpx-player-container') || document.querySelector('#movie_player');
  function close() {
    stopDrag();
  ...
  function size(value) {
    height = Math.max(mode==="immersive"?10:30, Math.min(mode==="immersive"?80:65, Number(value) || (mode==="immersive"?30:45)));
    document.documentElement.style.setProperty('--ytd-dock-height', mode==='immersive'?`max(96px, ${height}vh)`:height+'vh');
  }
  async function open(preferenceHeight, requestedMode="vertical") {
    if(host?.isConnected && requestedMode===mode)return {success:true};
    close();mode="immersive";
    ...
    document.body.append(host);YTD_LAYOUT_TOOLBAR.apply(YTD_LAYOUT_TOOLBAR.css);clearBlankOverride();return {success:true};
  }
  function fullscreen(){
