...
(() => {
  ...
  let host, sheet, player, timer, frameObserver, ancestors = [], height = 45;
  ...
  function size(value) {
    height = Math.max(30, Math.min(65, Number(value) || 45));
    document.documentElement.style.setProperty('--ytd-dock-height', height + 'vh');
  }
  async function open(preferenceHeight) {
    ...
    document.documentElement.setAttribute('data-ytd-layout','vertical'); size(preferenceHeight);
    ...
    const frame=document.createElement('iframe');frame.title='视频英语学习';frame.src=chrome.runtime.getURL('sidepanel.html?embedded=1');frame.allow='clipboard-write';shadow.append(frame);
    ...
    const saveSize=()=>chrome.runtime.sendMessage({action:'saveLayoutHeight',height});
  ...
  const message=(m,sender,reply)=>{
    ...
    if(m.action==='openVerticalLayout'){open(m.height).then(reply,e=>reply({success:false,error:e.message}));return true;}
  ...
  function cleanup(){close();clearInterval(timer);chrome.runtime.onMessage.removeListener(message);}
