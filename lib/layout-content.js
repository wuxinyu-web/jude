...
(() => {
  ...
  let host, sheet, player, timer, frameObserver, ancestors = [], height = 45, mode="vertical", stopDrag=()=>{}, inlineVideo=null, hadInline=false;
  const playerNode = () => document.querySelector('#bilibili-player') || document.querySelector('#bilibiliPlayer') || document.querySelector('.bpx-player-container') || document.querySelector('#movie_player') || document.querySelector('#player-container-id') || document.querySelector('video')?.parentElement;
  function close() {
    stopDrag();
    if(inlineVideo&&!hadInline)inlineVideo.removeAttribute('playsinline');inlineVideo=null;
  ...
  function size(value) {
    height = Math.max(["immersive","mobile"].includes(mode)?10:30, Math.min(["immersive","mobile"].includes(mode)?80:65, Number(value) || (mode==="immersive"?30:45)));
    document.documentElement.style.setProperty('--ytd-dock-height', mode==='mobile'?`max(160px, ${height}dvh)`:mode==='immersive'?`max(96px, ${height}vh)`:height+'vh');
  }
  async function open(preferenceHeight, requestedMode="vertical") {
    if(host?.isConnected && requestedMode===mode)return {success:true};
    close();mode=requestedMode==="mobile"?"mobile":"immersive";
    const node = playerNode();
    if (!node || node===document.body || !YTD_PLATFORM.videoIdFromUrl(location.href)) return {success:false,error:'播放器尚未准备好，请稍后再点击英语学习。'};
    if (host?.isConnected) return {success:true};
    inlineVideo=node.querySelector('video');if(inlineVideo){hadInline=inlineVideo.hasAttribute('playsinline');inlineVideo.setAttribute('playsinline','');}
    ...
    sheet = document.createElement('style');sheet.textContent = `
      ...
      html[data-ytd-layout=vertical] body *:not([data-ytd-layout-player]):not([data-ytd-layout-player] *){visibility:hidden!important}
      ...
      html[data-ytd-layout=vertical] [data-ytd-layout-player]{visibility:visible!important}
      ...
      /* YouTube's video wrapper can be zero-height; percentage video sizing
         needs a definite wrapper and must replace native pixel offsets. Keep
         native descendant visibility intact (hidden overlays must stay hidden). */
      html[data-ytd-layout=vertical] [data-ytd-layout-player] .html5-video-container{position:absolute!important;inset:0!important;width:100%!important;height:100%!important}
      html[data-ytd-layout=vertical] [data-ytd-layout-player] .html5-video-container video{position:absolute!important;inset:0!important;margin:0!important;max-width:none!important;max-height:none!important}
      ...
      html[data-ytd-immersive] .bpx-player-sending-bar,html[data-ytd-immersive] .bpx-player-dm-wrap,html[data-ytd-immersive] #ytd-note-button,html[data-ytd-immersive] #ytd-note-button-${chrome.runtime.id}{display:none!important}
      @media (max-width:700px){
        html[data-ytd-layout=vertical] [data-ytd-layout-player]{height:calc(100dvh - var(--ytd-dock-height))!important}
      }
    ...
    if(mode==='immersive'){
      const full=document.createElement('button');full.id='dockFullscreen';full.textContent='全屏';full.title='视频和台词一起全屏';full.addEventListener('click',()=>fullscreen());shadow.querySelector('header').prepend(full);
      shadow.getElementById('resize').setAttribute('aria-valuemin','10');shadow.getElementById('resize').setAttribute('aria-valuemax','80');
    }
    const frame=document.createElement('iframe');frame.title='句得';frame.src=chrome.runtime.getURL('sidepanel.html?embedded=1'+(mode==='immersive'?'&immersive=1':'&mobile=1'));frame.allow='clipboard-write';shadow.append(frame);
    ...
    const saveSize=()=>chrome.runtime.sendMessage({action:'saveLayoutHeight',height,mode}).catch(()=>{});
    const updateSize=value=>{size(value);resize.setAttribute('aria-valuenow',String(Math.round(height)));};
    // A thin edge adds a generous grab target without adding another toolbar row.
    const edge=document.createElement('div');edge.id='resizeEdge';edge.setAttribute('aria-hidden','true');edge.title='上下拖动，调整视频与字幕比例';shadow.append(edge);
    let drag=null;
    stopDrag=()=>{
      if(!drag)return;
      const {target,id}=drag;drag=null;host?.removeAttribute('data-resizing');
      if(target.hasPointerCapture(id))target.releasePointerCapture(id);
      void saveSize();
    };
    for(const target of [resize,edge]){
      target.addEventListener('pointerdown',e=>{
        if(e.button!==0)return;
        e.preventDefault();resize.focus();
        drag={target,id:e.pointerId,y:e.clientY,pixels:host.getBoundingClientRect().height};
        host.setAttribute('data-resizing','');target.setPointerCapture(e.pointerId);
      });
      target.addEventListener('pointermove',e=>{
        if(drag?.id===e.pointerId)updateSize((drag.pixels+drag.y-e.clientY)/innerHeight*100);
      });
      for(const event of ['pointerup','pointercancel','lostpointercapture'])target.addEventListener(event,stopDrag);
      target.addEventListener('dblclick',()=>{updateSize(30);void saveSize();});
    }
    resize.title='上下拖动调整比例；双击恢复默认；方向键微调';
    resize.addEventListener('keydown',e=>{
      if(['ArrowUp','ArrowDown','Home'].includes(e.key)){
        e.preventDefault();updateSize(e.key==='Home'?30:height+(e.key==='ArrowUp'?5:-5));void saveSize();
      }
    });
    shadow.getElementById('close').addEventListener('click',close);
    if(mode==='mobile'){shadow.querySelector('span').textContent='句得 · 视频学习';const touch=document.createElement('style');touch.textContent='header{height:44px}button{min-height:44px}iframe{height:calc(100% - 44px)}#resizeEdge{position:absolute;top:-6px;height:12px;left:0;right:0;cursor:ns-resize;touch-action:none}:host([data-resizing]) iframe{pointer-events:none}';shadow.append(touch);}
    document.body.append(host);if(mode==='immersive')YTD_LAYOUT_TOOLBAR.apply(YTD_LAYOUT_TOOLBAR.css);clearBlankOverride();return {success:true};
  }
  function fullscreen(){
  ...
  function nativeFullscreen(e){
    if(!host || mode!=='immersive' || !document.documentElement.requestFullscreen)return;
