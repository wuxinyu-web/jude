var YTD_LAYOUT = (() => {
  ...
  async function vertical(tabId,requestedMode) {
    ...
    const result=await chrome.tabs.sendMessage(tabId,{action:'openVerticalLayout',mode:normalize(requestedMode||prefs?.mode),height:prefs?.immersiveHeight||30});
  ...
  chrome.runtime.onMessage.addListener((m,sender,reply)=>{
    ...
    if(m.action==='saveLayoutHeight') {
      if(sender.id!==chrome.runtime.id || !sender.tab?.id || !YTD_PLATFORM.videoIdFromUrl(sender.url) || !Number.isFinite(m.height) || m.height<(m.mode==='immersive'?10:30) || m.height>(m.mode==='immersive'?80:65)) {reply({success:false});return false;}
