...
if(typeof document!=='undefined' && /(?:[?&])immersive=1(?:&|$)/.test(location.search))(()=>{
  ...
  void chrome.runtime.sendMessage({action:'syncImmersiveToolbar'}).catch(()=>{});
  ...
  if(chrome.runtime.getURL('').startsWith('safari-web-extension:')){back.title='返回完整学习区';back.setAttribute('aria-label','返回完整学习区');}
  ...
  function synchronize(){
    ...
    document.documentElement.toggleAttribute('data-awaiting-english',chinese||c.partial||Boolean(globalThis.YTD_ASR_UI?.state().running));
    if((chinese||c.partial)&&automaticVideo!==c.videoId){automaticVideo=c.videoId;void globalThis.YTD_ASR_UI?.ensureAutomatic();}
  }
