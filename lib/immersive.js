...
if(typeof document!=='undefined' && /(?:[?&])immersive=1(?:&|$)/.test(location.search))(()=>{
  ...
  const asrRetry=button('重试转写','重试当前视频的英文原声转写',()=>YTD_ASR_UI.start()),asrCancel=button('取消转写','取消当前英文原声转写',()=>YTD_ASR_UI.cancel());asrRetry.hidden=asrCancel.hidden=true;
  toolbar.append(resume,star,bilingual,retryChinese,asrRetry,asrCancel,button('展开字幕 ↓','返回完整字幕和学习工具',()=>full('transcript')),button('收藏库','打开收藏库',()=>full('library')));
  ...
  function renderChinese(){
    chinese.hidden=!showChinese;retryChinese.hidden=true;asrRetry.hidden=asrCancel.hidden=true;if(!showChinese||!cue){chinese.textContent='';return;}
    if(/^(ai-)?zh(?:-|$)/i.test(P.context().language||'')){const a=globalThis.YTD_ASR_UI?.state();chinese.textContent=a?.message||'正在准备英文原声，完成后自动显示双语…';asrCancel.hidden=!a?.running;asrRetry.hidden=Boolean(a?.busy||a?.running);return;}
  ...
  let automaticVideo=null;
  document.addEventListener('ytdAsrStatus',()=>renderChinese());
  function render(detail){
    ...
    if(automaticVideo!==videoId && /^(ai-)?zh(?:-|$)/i.test(c.language||'')){automaticVideo=videoId;void globalThis.YTD_ASR_UI?.ensureAutomatic();}
    if(locked)return;
