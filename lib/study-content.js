/* Runs on supported video pages. Owns reversible distraction styles and playback samples. */
(() => {
  ...
  function update(s){
    ...
    if(video && s?.restorePosition && s.videoId===videoId()){video.currentTime=s.position||0;video.pause();}
  }
  async function pulse(leaving=false){
    if(busy)return;
    busy=true;
    try{
      const response=await chrome.runtime.sendMessage({action:"studyPulse",documentId,leaving,online:navigator.onLine,position:video?.currentTime||0,visible:document.visibilityState==="visible",
  ...
  function playback(event){
    ...
    if(event.type==="play" && session && session.status!=="ended"){
      if((session.reduceDistractions && session.videoId!==videoId()))event.target.pause();
    }
  ...
  window.addEventListener("online",()=>void pulse(),{signal});window.addEventListener("offline",()=>void pulse(),{signal});
  ...
  window.addEventListener("pagehide",()=>{void pulse(true);cleanup();},{once:true});bindVideo();void pulse();
})();
