...
(() => {
  ...
  let initialized=false,captureInstalled=false,leaveTimer=null,hoverTimer=null,hoverKey="",hoverPoint=null,float=null,dragging=false,lastPanelPing=0,studyPollBusy=false,panelOpened=false;
  const ctx=()=>P.context();
  ...
  function hideFloat(){clearTimeout(leaveTimer);leaveTimer=null;globalThis.CSS?.highlights?.delete("ytd-hover-word");clearTimeout(hoverTimer);hoverTimer=null;hoverKey="";state.captureGeneration++;float?.remove();float=null;}
  ...
  async function lookup(item,generation){
    ...
    const run=async()=>{
      ...
      try{
        ...
        const save=button(already?"已收藏":"收藏单词",async(_e,b)=>{box.dataset.pinned="true";b.disabled=true;try{const r=await send("saveVocabulary",item.meta);b.textContent=r.alreadySaved?"已收藏":"已收藏 ✓";await P.refreshVocabulary();void pollStudy();}catch(e){b.disabled=false;throw e;}} ,"learning-button primary");save.disabled=already;
  ...
  function installCapture(){
    ...
    document.addEventListener("pointercancel",()=>{dragging=false;hideFloat();});
