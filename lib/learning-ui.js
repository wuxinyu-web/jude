...
(() => {
  ...
  function openExport(){
    ...
    download=button("生成并下载",async()=>{
      ...
      try{const blob=await YTD_WORD.toBlob(entries,{mode});const url=URL.createObjectURL(blob),a=el("a");a.href=url;a.download=`YouTube-Digest-${mode}-${new Date().toISOString().slice(0,10)}.docx`;document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),60000);status.textContent="Word 已生成，请查看下载列表。";}
  ...
  function formatTime(ms){const sec=Math.floor((ms||0)/1000);return `${Math.floor(sec/60)}:${String(sec%60).padStart(2,"0")}`;}
  ...
  function renderStudy(){
    ...
    if(root.dataset.signature!==signature){
      root.dataset.signature=signature;root.replaceChildren(el("h2","Study · 专注与复习"),el("p","记录观看和复习行为，不把播放时长当作学习效果。","learning-muted"));
      if(!running){
    ...
    if(s){const labels={running:"学习中",paused:"已暂停",due:"时间到了",review:"复习中",ended:"本次已结束"};stats.append(el("p",`${labels[s.status]} · 剩余 ${formatTime(Math.max(0,s.targetMs-s.watchMs-s.activityMs))}`));
  ...
  function renderReview(){
    ...
    if(!e){root.append(el("h3","本轮复习完成"),el("p","“记住了”是你的自评，需要时可在 Library 中筛选“还要复习”。"));return;}
    ...
    if(!state.revealed)root.append(button("回忆后查看答案",()=>{state.revealed=true;renderReview();},"learning-button primary"));
      ...
      root.append(el("p",e.kind==="sentence"?e.translationZh||"解析未完成，可到 Library 重试。":e.meaningZh));
  ...
  async function init(){
    ...
    setInterval(()=>{void pingPanel();void pollStudy();},2000);
