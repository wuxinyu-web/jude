...
var YTD_PLATFORM = (() => {
  function biliParts(id){const m=String(id||'').match(/^(BV[0-9A-Za-z]{10})(?:_p([1-9][0-9]{0,3}))?$/);return m?{bvid:m[1],page:Number(m[2]||1)}:null;}
  function parse(url){
  ...
  function sourceUrl(id,seconds){
    ...
    if(parts){url=new URL(`https://www.bilibili.com/video/${parts.bvid}/`);if(parts.page>1)url.searchParams.set('p',parts.page);}
